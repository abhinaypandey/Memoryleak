<?php 
	...
	/*****************************database insertion script****************************/
		if($_SERVER['REQUEST_METHOD']=='POST')
		{
				...
				$p_major='';
				$p_skill='';
				$query_1='insert into projects(user_id,title,description,bounty, date_time) values("'.$p_user.'","'.$p_title.'","'.$p_desc.'","'.$p_bounty.'",now());';
				if(mysqli_query($con,$query_1)){
					if($result=mysqli_query($con,"select MAX(project_id) as project_id from projects")){
					...
					if(!($_FILES["p-file"]["error"] > 0))
					{
						...
						$extension = end($temp);
						$p_file=$row['project_id'].'.'.$extension;
					}
					else
					{
						$p_file='';
					}
					mysqli_query($con,'update projects set file="'.$p_file.'" where project_id="'.$row['project_id'].'";');	
					if(isset($_POST['p_skills']) && !empty($_POST['p_skills'])){
						foreach($p_skills as $p_skill){
							if(!(mysqli_query($con,'insert into project_skills values("'.$row['project_id'].'","'.$p_skill.'");'))){
					...
					if(isset($_POST['p_majors'])  && !empty($_POST['p_majors'])){
						foreach($p_majors as $p_major){
							if(!(mysqli_query($con,'insert into project_majors values("'.$row['project_id'].'","'.$p_major.'");'))){
					...
					move_uploaded_file($_FILES["p-file"]["tmp_name"],"../uploads/projects_files/" .$p_file);
                                        $result=mysqli_query($con,"select project_id from projects where user_id=$login_session and date_time=(select MAX(date_time) from projects where user_id=$login_session);");
					$row=mysqli_fetch_array($result);
					$currentProject=$row['project_id'];
					echo "<script>var currentProject=".$currentProject."</script>";
				 }
				else{
		...
		<script>
			...
			//Checking whether form submitted or not	
				$(function(){
					...
					if(check=='true'){
						$('.main-content').slideUp('ease-in-out');;
						$('#p-submit').remove();
						$('#msubmit').remove();
						$('#head-span').text('Your project has been uploaded successfully...');
						$('#mhead-span').text('Your project has been uploaded successfully...');
                                                window.location.href='view_project.php?project_id='+currentProject;
					}
				});
			...
			/*****************************Script for mobile****************************/
				...
				$('#mtitle').change(function(){
					...
					var tit_len=$('#mtitle').val().length;
							if(tit_len==''){
								$('#mtitle-span').text('Enter the title of the project');
				...
				$('#mbounty').change(function(){
					...
					if(bounty==''){
						...
						$('#mbounty-span').text('Please enter the price of your project');
				...
				$('#m-submit').click(function(e) {
					...
					$('span').css('color','#678889');
							...
							else if(tit_len<10 ||tit_len>120){
								$('#mtitle-span').text('Title should be in between 10-120');
								$('#mtitle-span').css('color','#900');
								$('#mtitle').css('background-color','#ffe6e6');
								flag_1=false;
							}
							...
							if(desc_len<5){
								$('#mdesc-span').text('Enter the description of your project');
							...
							if(file==''){
								$('#mfile-span').text('Upload your project file');
			...
			$(document).ready(function() {
				...
				tinymce.init(
						...
						//-------------------------------------validation--------------------------------------
							...
							$('#p-title').change(function(){
								...
								if(tit_len==''){
									...
									$('#title-span').text('Title should be in between 10-120');
									$('#title-span').css('color','#900');
									$('#p-title').css('background-color','#ffe6e6');
									flag_1=false;
								}
							...
							$('#p-bounty').change(function(){
								...
								if(bounty==''){
											...
											$('#bounty-span').text('Please enter the price of your project');
								...
								if(desc_len<5){
									$('#desc-span').text('Enter the description of your project');
						...
						$('#p-submit').click(function(e){
							...
							if(file==''){
								$('#file-span').text('Upload your project file');
								$('#file-span').css('color','#900');
								flag_4=false;
							}
							else{
		...
		<div class="body">
			<div class="content">
				<div class="main-wrapper">
                	<form class="upload-form" action="" enctype="multipart/form-data" method="post" id="upload-form1">	
                    	<div class="header">
                        	<h1>Project Upload</h1>
                            <span id="head-span">Upload your project and get earning...</span>
                        </div>
                       
					   <div class="main-content">
                            <input type="text" name="p-title" class="input title" id="p-title" placeholder="Title" style="width:98%;">
                            <span  id="title-span" class="span">10 - 120 characters</span>
						   ...
						   <div class="input" id="p-desc-container" style="padding:0.5%;">
                            ...
                            <span id="desc-span" class="span">Description about your Project.You can include screen shot of your works as it will enhance the presentation</span>
                            ...
                            <div class="file-name">
								...
								<span style="max-width:100px;" id="file-span" class="span">Upload the Project file. Only pdf , doc , png , jpg , bmp , zip files are supported.</span>
                        </div>
          ...
          <!--------------------------------------------------------------Mobile section-------------------------------------------------------------->
			<div class="mcontent">
				<div class="main-wrapper" id="mmain-wrapper">
                	<form class="upload-form" action="" enctype="multipart/form-data" method="post" id="mupload-form">	
                    	<div class="header">
                        	<h1>Project Upload</h1>
                            <span id="mhead-span">Upload your project and get earning...</span>
                         </div>
						<div class="main-content">
							...
							<div id='mtagcontainer2' >
                                ...
                                <span id="major-span" class="span">The main catagory of this Project . Eg:Programming and IT experts,Doctors,Marketers etc.</span>
							</div>
							...
							<span id="mdesc-span" class="span">Description about your Project.You can include screen shot of your works as it will enhance the presentation</span>
						   ...
						   <div class="new-file">Filename</div>
							...
							<span style="max-width:100px;" id="mfile-span" class="span">Upload the Project file. Only pdf , doc , png , jpg , bmp , zip files are supported.</span>
                        </div>

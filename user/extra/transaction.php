			...
			$(document).ready(function(){
				...
				$('input').click(function(){
					...
					if(count>=0&&count<=total){
						$.ajax({
							type:'POST',
							url:'/memoryleak/user/content/query.php',
							data:{ 
							type:2,
							count:count
							}
					...
					<table  style=" border-collapse:collapse; width:100%;">
						...
						<tbody id="tbody">
							<?php
								...
								$result=mysqli_query($con,'select * from temp order by date_time desc limit 10');
								$i=0;
								while( $row=mysqli_fetch_array($result)){
									...
									if($i%2==0)
										echo "<tr style='  height: 25px; background-color:#b4d4f4; border-bottom:1px solid grey;'><td>$tid</td><td>$email</td><td>$transaction</td><td>$$order_total</td><td>$date_time</td></tr>";
									else
										echo "<tr style=' height: 25px;  background-color:whitesmoke;'><td>$tid</td><td>$email</td><td>$transaction</td><td>$$order_total</td><td>$date_time</td></tr>";
									$i++;
								}
						...
						<tbody id="tbody">
							<?php
								...
								while( $row=mysqli_fetch_array($result)){
									...
									$date_time=$row['date_time'];
									if($i%2==0)
										echo "<tr style='  height: 25px; background-color:#b4d4f4; border-bottom:1px solid grey;'><td>$tid</td><td>$$order_total</td><td>$date_time</td></tr>";
									else
										echo "<tr style=' height: 25px;  background-color:whitesmoke;'><td>$tid</td><td>$$order_total</td><td>$date_time</td></tr>";
									$i++;
								}
